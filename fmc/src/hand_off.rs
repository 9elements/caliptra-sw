...
use caliptra_drivers::{Array4x12, KeyId, WarmResetEntry4, WarmResetEntry48};
...
impl HandOff {
    ...
    pub fn fmc_cdi(&self) -> KeyId {
        KeyId::try_from(self.fht.fmc_cdi_kv_idx).unwrap_or_else(|_| crate::report_error(0xdead))
    }

    /// Retrieve FMC Alias Private Key
    pub fn fmc_priv_key(&self) -> KeyId {
        KeyId::try_from(self.fht.fmc_priv_key_kv_idx)
